...
function calc (beta0,beta1,beta2,beta3,mHeight,fHeight,cHeight,cWeight) {
  // AlturaAdulta = 0M + 1M*AlturaNiño + 2M*PesoNiño + 3M*PromedioAlturaPadres 
...
function process (callerObj) {
  ...
  var mMtVal = gMakeNumber(callerObj.form.mFeet.value, 0);
  var mCmVal = gMakeNumber(callerObj.form.mInch.value, 0);
  var mNewMtVal = Math.floor(mCmVal/100);
  mCmVal = (mCmVal%100);
  callerObj.form.mInch.value=mCmVal;
  mMtVal+=mNewMtVal;

  var fMtVal = gMakeNumber(callerObj.form.fFeet.value, 0);
  var fCmVal = gMakeNumber(callerObj.form.fInch.value, 0);
  var fNewMtVal = Math.floor(fCmVal/100);
  fCmVal = (fCmVal%100);
  callerObj.form.fInch.value=fCmVal;
  fMtVal+=fNewMtVal;

  var cMtVal = gMakeNumber(callerObj.form.cFeet.value, 0);
  var cCmVal = gMakeNumber(callerObj.form.cInch.value, 0);
  var cNewMtVal = Math.floor(cCmVal/100);
  cCmVal = (cCmVal%100);
  callerObj.form.cInch.value=cCmVal;
  cMtVal+=cNewMtVal;

  var cW = gMakeNumber(callerObj.form.cWeight.value, 0)/0.4536;  //converts it to pounds

  if ((mMtVal <= 0) && (mCmVal <= 0)){
    alert('Parece haber un error en la altura de la madre');
    return;
  }
  if ((fMtVal <= 0) && (fCmVal <= 0)){
    alert('Parece haber un error en la altura del padre');
    return;
  }
  if ((age!="LT4") && (cMtVal <= 0) && (cCmVal <= 0)){
    alert('Parece haber un error en la altura del ni&ntilde;o');
    return;
  }
  if ((age!="LT4") && (cW <=0)){
    alert('Parece haber un error en el peso del ni&ntilde;o');
    return;
  }
  if (!gender){
    alert('Por favor, seleccione el se&ntildexo de su ni&ntilde;o');
    return;
  }

  var rs = 0;
  
  var mH = gToInches(mMtVal, mCmVal);
  var fH = gToInches(fMtVal, fCmVal);
  var cH = gToInches(cMtVal, cCmVal);
